...
- **Timed sessions**: tap the time-left line under the progress bar and choose *Read for* 5, 10, 15, 20 or 30 minutes. Each option shows where it will end ("to the end of Chapter 5"), because sessions stop at a natural break: a chapter end if one falls close to the time, otherwise a paragraph or sentence end, never mid-sentence. The session counts down while you read and ends with a short summary.
