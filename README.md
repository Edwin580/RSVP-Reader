...
- **Timed sessions**: tap the time-left line under the progress bar. *A set time*: pick 5–30 minutes or set any length up to 2 hours; Chapter shows where it will end ("Ends in Chapter 6") and stops at a natural break, a chapter end if one falls close to the time, otherwise a paragraph or sentence end, never mid-sentence. *To a chapter end*: read to the end of this chapter or one of the next few, each with how long it'll take. The session counts down while you read and ends with a short summary.
