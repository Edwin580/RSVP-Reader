...
- **Two reading modes**, switched in the Aa menu or with `M`:
  ...
  - **Page**: guided reading. The book is laid out as pages that fit your screen, a marker glides under each word at your chosen speed, and the page turns itself when you reach the end: the old page slides out as the new one slides in, the marker fades in on the first word, and that word gets a little extra time so your eyes can move to the top. Pages end at a sentence when one falls on the last two lines. Tap any word to jump there. Pages are measured to fit exactly and re-flow when you rotate or resize.
