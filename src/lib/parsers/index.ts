...
export async function parseFile(file: File, onProgress?: (fraction: number) => void): Promise<Book> {
  ...
  switch (ext) {
    case '.epub': {
      ...
      const { title, sections } = await parseEpub(data)
      request = { kind: 'sections', id, title: title || fallbackTitle, sections }
      break
    }
    case '.pdf': {
      const { parsePdf } = await import('./pdf')
      const { title, sections } = await parsePdf(data, onProgress)
      request = { kind: 'sections', id, title: title || fallbackTitle, sections }
      break
    }
    case '.txt':
